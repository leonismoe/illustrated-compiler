...
function update(text, rlg) {
  return new Promise((resolve, reject) => {
    ...
    Promise.all([
      ...
      $nfa.innerHTML = svg_nfa.slice(svg_nfa.indexOf('-->', 57) + 3).replace('<title>NFA</title>', ''); // remove <?xml...
      $dfa.innerHTML = svg_dfa.slice(svg_dfa.indexOf('-->', 57) + 3).replace('<title>DFA</title>', ''); // remove <?xml...
